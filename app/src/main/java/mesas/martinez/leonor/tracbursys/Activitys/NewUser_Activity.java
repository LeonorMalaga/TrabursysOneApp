...
import mesas.martinez.leonor.tracbursys.Services.NewSpeechBluService;
 ...
 */
public class NewUser_Activity extends ActionBarActivity  {
...
//------------------------------SubClass----------------------------------//
    ...
    protected void onResume( ) {
        super.onResume();
        serviceState=PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString(Constants.SERVICE_STATE, NewSpeechBluService.State.DISCONNECTING.name());
        if(!serviceState.equals(NewSpeechBluService.State.SCANNING.name()) && !serviceState.equals(NewSpeechBluService.State.WAIT.name())) {
            ...
            public void onClick(View view) {
               CharSequence text= stop_start.getText();
                if(text.equals(getResources().getString(R.string.start_service))){
                 ...
                 NewUser_Activity.this.startService();
                }else{
                   //stop Service and change the button text
                    Intent intent = new Intent(Constants.SERVICE_STOP);
                    LocalBroadcastManager.getInstance(NewUser_Activity.this).sendBroadcastSync(intent);
                    buttonText=getResources().getString(R.string.start_service);
                    stop_start.setText(buttonText);
                }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        serviceState = NewSpeechBluService.State.CONNECTING.name();
            PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
                .edit()
                .putString(Constants.SERVICE_STATE, serviceState)
                .commit();
        Intent intent = new Intent(Intent.ACTION_SYNC, null, this, NewSpeechBluService.class);
        startService(intent);
